...
class Orders{
	...
	Orders(String orderId,String customerId,String selectedSize,int quantity,double amount,int status){
...
class Example{
	...
	public static String generateId(){
		...
		String lastId=ordersArray[ordersArray.length-1].getOrderId();
	...
	public static void searchCustomer(){
		L3:do{
			...
			for(int i=0;i<ordersArray.length;i++){
				...
				if(phoneNumber.equals(ordersArray[i].getCustomerId())){	
					extendIndexArray();		
			...
			for(int i=0;i<indexArray.length;i++){
				totalQuantityArray[i]=ordersArray[indexArray[i]].getQuantity();
				totalSizeArray[i]=ordersArray[indexArray[i]].getSelectedSize();
			}
	...
	public static void searchOrder(){
		do{
			...
			L4:while(true){
				...
				for(int i=0;i<ordersArray.length;i++){
					if(ordID.equals(ordersArray[i].getOrderId())){
						System.out.println("\n");
						System.out.println("\t\tPhone Number : "+ordersArray[i].getCustomerId());
						System.out.println("\t\tSize : "+ordersArray[i].getSelectedSize());
						System.out.println("\t\tQTY : "+ordersArray[i].getQuantity());
						System.out.printf("\t\tAmount : %.2f \n",ordersArray[i].getAmount());
						if(ordersArray[i].getStatus()==0){
							System.out.println("\t\tStatus : PROCESSING");
						}else if(ordersArray[i].getStatus()==1){
							System.out.println("\t\tStatus : DELIVERING");
						}else if(ordersArray[i].getStatus()==2){
	...
	public static void bestInCustomers(){
		...
		for(int i=0;i<ordersArray.length;i++){	
			...
			for(int j=0;j<ordersArray.length;j++){			
				if(ordersArray[i].getCustomerId().equals(ordersArray[j].getCustomerId())){						
					arQuantity[arQuantity.length-1]+=ordersArray[j].getQuantity();											
					arAmount[arAmount.length-1]+=ordersArray[j].getAmount();
				}			
			}
		}
		...
		for(int i=0;i<ordersArray.length;i++){
			if(searchString(arCustomerId,ordersArray[i].getCustomerId())!=-1){
				continue;
			}
			arCustomerId=arrExtend(arCustomerId);
			arCustomerId[arCustomerId.length-1]=ordersArray[i].getCustomerId();
	...
	public static void allCustomerReports(){
		...
		for(int i=0;i<ordersArray.length;i++){
			...
			for(int j=0;j<ordersArray.length;j++){
				if(ordersArray[i].getCustomerId().equals(ordersArray[j].getCustomerId())){
					switch(ordersArray[j].getSelectedSize()){
						case "XS": XS[XS.length-1]+=ordersArray[j].getQuantity();
						break;
						case "S": S[S.length-1]+=ordersArray[j].getQuantity();
						break;
						case "M": M[M.length-1]+=ordersArray[j].getQuantity();
						break;
						case "L": L[L.length-1]+=ordersArray[j].getQuantity();
						break;
						case "XL": XL[XL.length-1]+=ordersArray[j].getQuantity();
						break;
						case "XXL": XXL[XXL.length-1]+=ordersArray[j].getQuantity();
					}
				}
			}
		} 
	...
	public static void sortedByQty(){
		...
		for(int i=0;i<ordersArray.length;i++){
			switch(ordersArray[i].getSelectedSize()){
				case "XS": drQuantityArray[0]+=ordersArray[i].getQuantity();
				break;
				case "S": drQuantityArray[1]+=ordersArray[i].getQuantity();
				break;
				case "M": drQuantityArray[2]+=ordersArray[i].getQuantity();
				break;
				case "L": drQuantityArray[3]+=ordersArray[i].getQuantity();
				break;
				case "XL":drQuantityArray[4]+=ordersArray[i].getQuantity();
				break;
				case "XXL":drQuantityArray[5]+=ordersArray[i].getQuantity();
			}
		}
	...
	public static void allOrders(){
		...
		String[] arr=new String[ordersArray.length];
		String[] brr=new String[ordersArray.length];
		String[] crr=new String[ordersArray.length];
		int[] drr=new int[ordersArray.length];
		double[] err=new double[ordersArray.length];
		int[] frr=new int[ordersArray.length];
		
		for(int i=0;i<arr.length;i++){
			arr[i]=ordersArray[i].getOrderId();
			brr[i]=ordersArray[i].getCustomerId();
			crr[i]=ordersArray[i].getSelectedSize();
			drr[i]=ordersArray[i].getQuantity();
			err[i]=ordersArray[i].getAmount();
			frr[i]=ordersArray[i].getStatus();
		}	
			...
			L5:while(true){
				...
				for(int i=0;i<ordersArray.length;i++){
					if(ordID.equals(ordersArray[i].getOrderId())){
						...
						switch(ordersArray[i].getStatus()){
							case 0:
							System.out.println("\n");
	...
	public static void removeOrder(int index){
		...
		for(int i=index;i<tempOrdersArray.length;i++){
			tempOrdersArray[i]=ordersArray[i+1];			
		}
